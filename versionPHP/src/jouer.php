...
$joueur1 = new Joueur ($idUser,$pays, $langue);
 // juste pour tester mais cette sera a saisir pa le user
	...
	echo" le pays du CPU est ".$cpu->getPays(). "et compte ".strlen($cpu->getPays())." lettres";
	...
	*/
	...
	echo"<br>Juge dis: le pays de ".$joueur1->getPseudo(). "a" .strlen($joueur1->getPays())." lettres et  celui de CPU a ". strlen($cpu->getPays())." lettres<br>";
...
$i =0;
	while($i<10)
	{
		...
		if(strlen($requete)!= 1)
		{
			if($reponse)
			{
				$jeu1->setVainqueur($joueur1->getPseudo());
				echo "<br>".$joueur1->getPseudo()." a trouvé le pays de CPU et a gagné ";
				break;
			}
			else
			{
				echo "<br> CPU dit : Mon pays n'est pas ".$requete;
			}
		}
		else if($reponse)
		{
		...
		if($cpu->getpaysIdentifie())
		{
			...
			if($guess == $joueur1->getPays())
			{
				...
				echo "<br>CPU a trouve ton pays ".$guess.", tu as perdu, son pays etait ".$cpu->getPays();
                break;
			}
			 #if it is not correct, then echo a message and continue
			else
			{
				echo "<br>CPU demande : Est-ce Ton Pays ".$guess;
                echo "<br>Judge dit : Non, ce n'est pas son pays. CPU passe la main";
                continue;
			}
		}
